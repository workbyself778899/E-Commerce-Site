...
useEffect(()=>{
  const getUser = async()=>{  
    try {
    const res = await axios.get(`https://e-commerce-site-5h4d.vercel.app/user/details/${id}`)
...
useEffect(() => {
  const fetchOrders = async () => {
    try {
      ...
      const res = await axios.get('https://e-commerce-site-5h4d.vercel.app/order/user', { headers: { 'auth-token': token } })
