...
const AdminBlog = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      let res;
      if (editing) {
        res = await axios.put(`http://localhost:3900/blog/edit/${editing}`, formData, {
          headers: { 'auth-token': token, 'Content-Type': 'multipart/form-data' }
        });
        toast.success(res.data.message || 'Blog updated');
        setEditing(null);
      } else {
        res = await axios.post('http://localhost:3900/blog/create', formData, {
          headers: { 'auth-token': token, 'Content-Type': 'multipart/form-data' }
        });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:3900/blog/delete/${id}`, {
